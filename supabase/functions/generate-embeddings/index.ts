...
async function needsReembedding(
...
serve(async (req) => {
  ...
  try {
    ...
    if (chunkId) {
      ...
      const needsRegen = forceRegenerate || await needsReembedding(
        chunk.embedding_model,
        chunk.embedding_model_version
      );

      if (!needsRegen && chunk.embedding) {
        result.skipped = 1;
      ...
      const textToEmbed = text || document.extracted_text || document.summary || '';
      ...
      const docNeedsRegen = forceRegenerate || await needsReembedding(
        document.embedding_model,
        document.embedding_model_version
      );
      ...
      const { data: chunks, error: chunksError } = await supabase
        .from('chunks')
        .select('id, content, embedding_model, embedding_model_version, embedding')
        .eq('document_id', documentId);
      ...
      if (!chunksError && chunks && chunks.length > 0) {
        ...
        for (const chunk of chunks) {
          ...
          const chunkNeedsRegen = forceRegenerate || await needsReembedding(
            chunk.embedding_model,
            chunk.embedding_model_version
          );
